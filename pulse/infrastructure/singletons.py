...
Usage — active path (unchanged):
    from pulse.infrastructure.singletons import cache, telemetry, enhanced_process_manager
    from pulse.infrastructure.singletons import ROCKYOU_PATH, COMMON_DIRB_PATH

Usage — intelligence layer (lazy):
    from pulse.infrastructure.singletons import get_cve_intelligence
...
def get_exploit_generator():
    ...
    if _exploit_generator is None:
        from pulse.intelligence.ai_exploit_generator import AIExploitGenerator
...
def get_vulnerability_correlator():
    ...
    if _vulnerability_correlator is None:
        from pulse.intelligence.vulnerability_correlator import VulnerabilityCorrelator
...
# These allow existing code that does `from pulse.infrastructure.singletons import X`
...
def __getattr__(name: str):
    ...
    raise AttributeError(f"module 'pulse.infrastructure.singletons' has no attribute {name!r}")
