"""
pulse.tools/bugbounty_engine.py
...
from pulse.infrastructure.singletons import get_bugbounty_manager
...
async def _execute_bb_phase(
    ...
    async def _run_tool(tool_name: str, params: Dict) -> Dict:
        ...
        try:
            from pulse.interface.server_setup import run_security_tool, _normalize_tool_result
            from pulse.tools.null_context import NullContext

            exec_params = {"target": target_domain, **params}
            null_ctx = NullContext()
            raw = await run_security_tool(null_ctx, tool_name, exec_params)
