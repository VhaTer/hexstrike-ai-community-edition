...
class TestMonitorSystemResources:
    def test_returns_resource_dict(self, pm):
        with patch("pulse.infrastructure.telemetry.performance_monitor.psutil.cpu_percent",
                   return_value=45.0), \
             patch("pulse.infrastructure.telemetry.performance_monitor.psutil.virtual_memory") as vm, \
             patch("pulse.infrastructure.telemetry.performance_monitor.psutil.disk_usage") as du, \
             patch("pulse.infrastructure.telemetry.performance_monitor.psutil.net_io_counters") as net:
