...
def test_confirmation_accepted_executes_and_records_telemetry():
    with patch("pulse.tools.wifi_direct.wifi_exec", return_value={"success": True, "output": "ok"}) as mock_exec, \
         patch("pulse.interface.server_setup.confirm_destructive_action", new_callable=AsyncMock, return_value=True) as mock_confirm, \
...
def test_success_writes_cache_and_records_metrics():
    ...
    with patch("pulse.tools.net_scan_direct.net_scan_exec", return_value=fake_output) as mock_exec, \
         patch("pulse.interface.server_setup._op_metrics.record") as mock_record:
        mcp = make_mcp()
...
def test_rate_limit_sets_state_and_records_telemetry():
    ...
    with patch("pulse.tools.net_scan_direct.net_scan_exec", return_value=fake_output), \
         patch("pulse.interface.server_setup._rate_limiter.detect_rate_limiting",
...
def test_optimizer_forced_stealth_applies_and_is_recorded():
    ...
    with patch("pulse.tools.net_scan_direct.net_scan_exec", return_value=fake_output) as mock_exec, \
         patch("pulse.interface.server_setup._optimizer.optimize") as mock_optimize, \
         patch("pulse.interface.server_setup._op_metrics.record") as mock_record:
