...
@pytest.fixture
def manager():
    with patch("pulse.infrastructure.execution.enhanced_process_manager.ResourceMonitor"), \
         patch("pulse.infrastructure.execution.enhanced_process_manager.ProcessPool"), \
         patch("pulse.infrastructure.execution.enhanced_process_manager.AdvancedCache"), \
         patch("pulse.infrastructure.execution.enhanced_process_manager.PerformanceDashboard"), \
         patch("pulse.infrastructure.execution.enhanced_process_manager.threading.Thread"):
...
class TestMonitorSystem:
    def test_monitor_system_normal(self, manager):
        ...
        with patch.object(manager, '_auto_scale_based_on_resources') as mock_auto_scale, \
             patch("pulse.infrastructure.execution.enhanced_process_manager.time.sleep", side_effect=[None, KeyboardInterrupt]):
    ...
    def test_monitor_system_exception(self, manager):
        with patch.object(manager, '_auto_scale_based_on_resources') as mock_auto_scale, \
             patch("pulse.infrastructure.execution.enhanced_process_manager.time.sleep", side_effect=[Exception("monitor error"), KeyboardInterrupt]):
            with pytest.raises(KeyboardInterrupt):
                manager._monitor_system()
        mock_auto_scale.assert_not_called()
